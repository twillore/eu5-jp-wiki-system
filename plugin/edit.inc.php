...
function plugin_edit_write()
{
	...
	if ($postdata === '') {
		...
		// --- ▼▼▼ ここから追記 ▼▼▼ ---
		require_once(LIB_DIR . 'discord.cls.php');
		$discord = new discord();
		$editor = !empty($_SERVER['PHP_AUTH_USER']) ? $_SERVER['PHP_AUTH_USER'] : '名無しさん';
		$message = 'ページが削除されました: ' . $page;
		$discord->notice($message, $editor . ' via PukiWiki');
		// --- ▲▲▲ ここまで追記 ▲▲▲ ---
	...
	// ページのパス部分を取得
	$page_path = get_page_uri($page, PKWK_URI_ROOT);
	// ベースURLとパスを結合して、完全なURLを作成
