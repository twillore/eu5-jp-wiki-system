...
//
// datatable.inc.php - High-performance interactive data table plugin.
//
...
// #datatable             - The main engine block plugin.
// #datatable_config      - Defines the start of the configuration block.
// #datatable_endconfig   - Defines the end of the configuration block.
// #datatable_data        - Defines the start of the data block.
// #datatable_enddata     - Defines the end of the data block.

require_once(LIB_DIR . 'convert_html.php');

// ============================================================
// ★★★ MAIN PLUGIN: #datatable ★★★
// ============================================================

// ============================================================
//  メイン関数 (#datatable) - 統合管制室
// ============================================================

/**
 * @brief メインプラグイン関数 (#datatable)
 * @details データ抽出、HTML/JS/CSSの構築、そして最終的な出力への結合を行うコントローラーとして機能します。
 * @en Main plugin function for #datatable. Acts as the controller that fetches data, builds HTML, JS, and CSS, and combines them into the final output.
...
function plugin_datatable_convert()
{
    ...
    list($json_data, $config, $settings, $debug_info) = plugin_datatable_get_data_and_config($data_page, $debug_mode);
    ...
    $html = plugin_datatable_get_template($config, $settings);
    $js_core = plugin_datatable_get_js_core($json_data, $config, $settings);
    $js_events = plugin_datatable_get_js_events($config);
    $js = plugin_datatable_get_js_wrapper($js_core . $js_events, $debug_mode);
    $css  = plugin_datatable_get_css();
 ...
 */
function plugin_datatable_get_data_and_config($data_page, $is_debug)
{
 ...
 */
function plugin_datatable_get_template($config, $settings)
{
 ...
 */
function plugin_datatable_get_js_wrapper($script_body, $is_debug)
{
 ...
 */
function plugin_datatable_get_js_core($json_data, $config, $settings)
{
    $config_json = json_encode($config); $settings_json = json_encode($settings);
    return <<<EOD
        data = {$json_data};
        const config = {$config_json}; const settings = {$settings_json};
        const listBody = document.querySelector('#datatable-engine .list');
        let tableHtml = '';
        data.forEach(item => {
            tableHtml += '<tr>';
        ...
        list = new List('datatable-engine', options); 
EOD;
}
 ...
 */
function plugin_datatable_get_js_events($config)
{
    ...
    return <<<EOD
        ...
        function setupGroupBy() {
            const groupBySelect = document.querySelector('#datatable-engine .group-by-select');
            groupBySelect.addEventListener('change', function() {
                currentGroupKey = this.value; sortState = {};
                document.querySelectorAll('#datatable-engine .sort').forEach(b => b.classList.remove('asc', 'desc'));
                performMasterSort();
            });
        ...
        function setupFilters() {
            const activeFilters = {};
            document.querySelectorAll('#datatable-engine .header-content').forEach(header => {
        ...
        function setupSorting() {
            document.querySelectorAll('#datatable-engine .sort').forEach(button => {
        ...
        function setupPaginationControls() {
            const paginationSelect = document.querySelector('#datatable-engine .pagination-select');
...
/**
 * @brief #datatable_config - 設定ブロックの開始マーカーを出力します。
 * @en #datatable_config - Renders the start marker for the config block.
 ...
 */
// #datatable_config - Renders <!-- DATATABLE_CONFIG_START -->
function plugin_datatable_config_convert() { return '<!-- DATATABLE_CONFIG_START -->'; }

// #datatable_endconfig - Renders <!-- DATATABLE_CONFIG_END -->
function plugin_datatable_endconfig_convert() { return '<!-- DATATABLE_CONFIG_END -->'; }

// #datatable_data - Renders <!-- DATATABLE_DATA_START -->
function plugin_datatable_data_convert() { return '<!-- DATATABLE_DATA_START -->'; }

// #datatable_enddata - Renders <!-- DATATABLE_DATA_END -->
function plugin_datatable_enddata_convert() { return '<!-- DATATABLE_DATA_END -->'; }
