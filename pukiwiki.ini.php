...
$page_title = 'EU5日本語wiki';

// Specify PukiWiki URL (default: auto)
$script = 'https://eu5.paradoxwiki.org/';

// Shorten $script: Cut its file name (default: not cut)
//$script_directory_index = 'index.php';

// Site admin's name (CHANGE THIS)
$modifier = 'EU5日本語wiki編集部';

// Site admin's Web page (CHANGE THIS)
$modifierlink = 'https://discord.gg/VcEuJpEBEH';
...
// Default: always fail
// 外部ファイルを読み込む
require_once('pukiwiki.secret.php');
...
$cycle  =   24; // Wait N hours between backup (0 = no wait)
$maxage = 30; // Stock latest N backups
...
// ▼▼▼ ここから追加コード (:config/IgnoreList 読み込み機能) ▼▼▼

// 除外リストを管理するページ名 (:config/IgnoreList) のファイルパス
// bin2hexで作った小文字を、strtoupperで大文字に変換する
$ignore_file_path = DATA_DIR . strtoupper(bin2hex(':config/IgnoreList')) . '.txt';

if (file_exists($ignore_file_path)) {
    // ファイルを読み込む
    $lines = file($ignore_file_path, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
    ...
    foreach ($lines as $line) {
        ...
        // リスト記法(- )やWikiリンク記法([[]])を除去してページ名だけ抽出
        $page_name = preg_replace('/^[-・\s]+|\[\[|\]\]/', '', $line);
        $page_name = trim($page_name);

        if ($page_name !== '') {
            // 正規表現用にエスケープして配列に追加（完全一致させるため ^と$ をつける）
            // preg_quoteの第2引数にデリミタ'/'を指定
            $ignore_patterns[] = '^' . preg_quote($page_name, '/') . '$';
        }
    }

    // 既存の除外設定に追加結合
    if (!empty($ignore_patterns)) {
        $non_list .= '|' . implode('|', $ignore_patterns);
    }
}
// ▲▲▲ ここまで追加コード ▲▲▲
